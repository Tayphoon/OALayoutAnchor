...
@implementation NSView (LayoutGuide)
...
+ (void)addLayoutGuideMethodsIfNeeded {
    BOOL hasMethods = [NSView instancesRespondToSelector:@selector(layoutMarginsGuide)];
    
    // If the methods are already available on NSView that means addLayoutGuideMethodsIfNeeded has already been called
    ...
    if (!hasMethods) {
        ...
        for (NSString *methodName in methods) {
            [self addOrCreateMethodWithName:methodName toClass:[NSView class]];
        }
    }
}
...
- (OALayoutGuide*)___layoutMarginsGuide {
    ...
    if (!layoutGuide) {
        layoutGuide = [[NSView layoutMarginsGuideClass] layoutGuideForView:self];
