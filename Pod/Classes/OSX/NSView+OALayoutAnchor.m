...
@import ObjectiveC;
#import "NSView+OALayoutAnchor.h"
...
@implementation NSView (OALayoutAnchor)
#ifndef __IPHONE_9_0
...
+ (void)addAnchorMethodsIfNeeded {
  BOOL hasMethods = [NSView instancesRespondToSelector:@selector(leadingAnchor)];
  
  // If the methods are already available on NSView that means addAnchorMethodsIfNeeded has already been called
  ...
  if (!hasMethods) {
    ...
    for (NSString *methodName in methods) {
      [self addMethodWithName:methodName toClass:[NSView class]];
    }
  }
